...
const BuildingCard = ({ building }) => (
  <div className="building-card">
    ...
    <div className="b-info">
      ...
      <p>Вартість: {building.cost}</p>
    </div>
  </div>
);

export default function MyCity({ resources, satisfaction, mapBuildings, syncWithDB }) {
  ...
  const upgradeMapBuilding = () => {
    ...
    if (resources.budget >= target.costBudget && resources.concrete >= target.costConcrete) {
      ...
      const newResources = { 
        budget: resources.budget - target.costBudget, 
        concrete: resources.concrete - target.costConcrete 
      };
      const newSatisfaction = Math.min(satisfaction + 5, 100);
      const newBuildings = mapBuildings.map(b => 
        ...
        <div className="city-grid-react">
          {cityMap.map((row, rowIndex) => 
            row.map((cell, colIndex) => {
              ...
              return <div key={`${rowIndex}-${colIndex}`} className={`cell ${cell}`}>{icon}</div>;
            })
          )}
        </div>
            ...
            <div>
              ...
              {!selectedBuildingData.upgraded ? (
                <p style={{fontSize: '0.85rem', color: '#f59e0b', marginBottom: '10px'}}>
                  Ціна: ${selectedBuildingData.costBudget.toLocaleString()} | {selectedBuildingData.costConcrete} т бетону
                </p>
              ) : (
